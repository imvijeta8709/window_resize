...
function App() {
  ...
  const fetchData = async () => {
    try {
      ...
      // Count the number of created and updated records
      const created = activeRecords.filter((record) => record.created).length;
      const updated = activeRecords.filter((record) => record.updated).length;
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      if (editingRecord) {
        ...
        setUpdatedCount(updatedCount + 1); // Increment updated count
        setEditingRecord(null);
      } else {
        await axios.post("http://localhost:8080/user/create", data);
        toast.success("Record added successfully");
        setCreatedCount(createdCount + 1); // Increment created count
      }
      fetchData();
    ...
    <SplitPane split="horizontal" size="50%">
      <SplitPane split="vertical" size="50%">
        <div className="container">
          ...
          <div className="text">Number of Records Created: {createdCount}</div>
          <div className="text">Number of Records Updated: {updatedCount}</div>
        </div>
        {showForm ? (
